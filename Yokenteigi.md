...
以下の条件で、小学生向けの4択問題を作成してください：

対象：小学4年生
教科：理科
単元：水のすがた（固体・液体・気体、水の循環）
問題数：30問
難易度：easy

要件：
1. 各問題は以下の要素を必ず含めてください：
   - 問題文
   - 4つの選択肢
   - 正解の選択肢番号（0-3）
   - ヒント（児童が自力で考えるためのアドバイス）
   - 解説（なぜその答えになるのかの詳しい説明）
...
3. 問題レベル：
   - 易しめの表現を使用
   - 1問につき2つ以上の概念を組み合わせない
   - 計算が必要な場合は簡単な足し算・引き算程度まで
...
- 重複する問題は作成しない
- 教科書レベルの正確な用語を使用
- 小学生が理解しやすい、簡潔な日本語で記述
- 具体的な数値を用いる場合は、計算が容易な数値を使用